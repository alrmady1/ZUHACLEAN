...
import { Plus, X, Eye, Trash2, Pencil, Check, FileText, Printer, Receipt } from 'lucide-react';
import { api } from '../lib/api.js';
import type { Appointment, Contract, Customer, Service, Quote } from '../../shared/types.js';
...
export default function Contracts() {
  ...
  const [mainTab, setMainTab] = useState<'contracts' | 'quotes'>('contracts');
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-xl font-bold text-slate-800">{mainTab === 'contracts' ? t('العقود الدورية') : t('عروض الأسعار')}</h1>
          <p className="text-sm text-slate-400">
            {mainTab === 'contracts'
              ? t('تُولَّد الزيارات تلقائياً في جدول المواعيد عند إنشاء العقد')
              : t('إنشاء عرض سعر لعميل قبل الالتزام بموعد أو عقد')}
          </p>
        </div>
        {mainTab === 'contracts' && canCreateContract && (
          <button
      ...
      <div className="flex items-center gap-1 rounded-xl border border-slate-200 bg-white p-1 w-fit">
        <button
          ...
          className={`flex items-center gap-1.5 rounded-lg px-4 py-1.5 text-sm font-medium ${mainTab === 'contracts' ? 'bg-brand-50 text-brand-700' : 'text-slate-500'}`}
        >
          <FileText className="h-4 w-4" /> {t('العقود')}
        </button>
        <button
          onClick={() => setMainTab('quotes')}
          className={`flex items-center gap-1.5 rounded-lg px-4 py-1.5 text-sm font-medium ${mainTab === 'quotes' ? 'bg-brand-50 text-brand-700' : 'text-slate-500'}`}
        >
          <Receipt className="h-4 w-4" /> {t('عرض سعر')}
        </button>
      </div>
