...
export interface LandingPageSettings {
  ...
  tagline: string;
  // حقل قديم — كان يتحكم بشريط "لا تشيل هم الدفع! يمكنك التقسيط عن طريق
  // تابي وتمارا" أسفل الهيرو مباشرة. حلّ محله show_tabby_tamara_banner
  // أدناه (نفس الشريط بالضبط)، ويبقى هذا الحقل فقط كقيمة احتياطية لمن
  // ضبطه صراحةً قبل إضافة الحقل الجديد (انظر التراجع show_tabby_tamara_
  // banner ?? show_installments_banner في OrderPage.tsx) — لا يُعرَض في
  // واجهة الإعدادات بعد الآن.
  show_installments_banner?: boolean;
  // إظهار/إخفاء شريط "لا تشيل هم الدفع! يمكنك التقسيط عن طريق تابي
  // وتمارا" (شعارا تابي وتمارا فقط) أسفل قسم الهيرو مباشرة في صفحة "اطلب
  // الخدمة" العامة — مستقل تماماً عن show_payment_methods_section أدناه.
  // افتراضياً ظاهر (true)، ويتراجع لقيمة show_installments_banner القديم
  // إن كان هذا الحقل غائباً (حسابات ضبطت الخيار القديم قبل هذا التحديث).
  show_tabby_tamara_banner?: boolean;
  // إظهار/إخفاء قسم "طرق الدفع" الكامل (مدى، Apple Pay، تابي، تمارا)
  // أسفل صفحة "اطلب الخدمة" العامة، فوق الفوتر مباشرة — مستقل تماماً عن
  // show_tabby_tamara_banner أعلاه. افتراضياً ظاهر (true).
  show_payment_methods_section?: boolean;
